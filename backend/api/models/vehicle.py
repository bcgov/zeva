...
class Vehicle(Auditable):
    ...
    class Meta:
        db_table = 'vehicle'
        unique_together = [[
            'make', 'model', 'vehicle_class_code', 'vehicle_fuel_type',
            'model_year'
        ]]
...
class VehicleChangeHistory(Auditable):
    ...
    previous_state = EnumField(
        VehicleDefinitionStates,
        max_length=20,
        null=False,
        default=VehicleDefinitionStates.NEW,
        db_comment='The previous review state of this vehicle. Valid states: '
                   '{states}'.format(
                       states=[c.name for c in VehicleDefinitionStates]
                    )
    )
    current_state = EnumField(
        ...
        null=False,
        default=VehicleDefinitionStates.NEW,
        db_comment='The review state of this vehicle. Valid states: '
                   '{states}'.format(
                       states=[c.name for c in VehicleDefinitionStates]
                    )
    )
