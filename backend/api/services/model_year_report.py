...
def get_model_year_report_statuses(report, request=None):
    ...
    if report.validation_status == ModelYearReportStatuses.SUBMITTED or \
        ...
        user_mixin_instance = UserMixin()
        user_mixin_instance.context = {'request': request} 
        user_serializer = user_mixin_instance.get_user_data(report, 'create_user')
    ...
    if report.validation_status == ModelYearReportStatuses.ASSESSED:
        ...
        user_mixin_instance = UserMixin()
        user_mixin_instance.context = {'request': request} 
        user_serializer = user_mixin_instance.get_user_data(report, 'update_user')
        assessment_confirmed_by = {
